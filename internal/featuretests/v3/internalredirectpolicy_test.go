...
func TestInternalRedirectPolicy_HTTProxy(t *testing.T) {
	...
	proxyRedirect := fixture.NewProxy("simple").WithSpec(
		contour_api_v1.HTTPProxySpec{
			VirtualHost: &contour_api_v1.VirtualHost{Fqdn: "hello.world"},
			Routes: []contour_api_v1.Route{{
				Services: []contour_api_v1.Service{{
				...
				InternalRedirectPolicy: &contour_api_v1.HTTPInternalRedirectPolicy{
					...
					Predicates: []contour_api_v1.HTTPInternalRedirectPredicate{
						{
							Name: contour_api_v1.SafeCrossScheme,
						},
						{
							Name: contour_api_v1.PreviousRoutes,
						},
						{
							Name:              contour_api_v1.AllowListedRoutes,
							AllowedRouteNames: []string{"default"},
						},
					},
	...
	rh.OnUpdate(proxy, proxyRedirect)
