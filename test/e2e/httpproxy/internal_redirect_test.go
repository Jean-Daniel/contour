...
func testInternalRedirectPolicy(namespace string) {
	Specify("internal redirect policy", func() {
		...
		proxy := getInternalRedirectHTTPProxy(namespace, true)
...
func doInternalRedirectTest(namespace string, proxy *contour_api_v1.HTTPProxy, t GinkgoTInterface) {
	...
	// /internal-redirect-denied check AllowedRouteNames predicate properly works
	assertInternalRedirectRequest(t, proxy.Spec.VirtualHost.Fqdn, "/internal-redirect-denied",
		"http://internalredirectpolicy.projectcontour.io/echo", 302)
...
func getInternalRedirectHTTPProxy(namespace string, removeServices bool) *contour_api_v1.HTTPProxy {
			...
			Routes: []contour_api_v1.Route{
				...
				// Route that returns a 302 redirect to the /echo route
				{
					...
					RequestRedirectPolicy: &contour_api_v1.HTTPRequestRedirectPolicy{
						Hostname:   pointer.StringPtr(fqdn),
						StatusCode: pointer.Int(302),
						Path:       pointer.StringPtr("/echo"),
					},
				},
				{
					Conditions: []contour_api_v1.MatchCondition{{
					...
					InternalRedirectPolicy: &contour_api_v1.HTTPInternalRedirectPolicy{
						Predicates: []contour_api_v1.HTTPInternalRedirectPredicate{{
							Name: contour_api_v1.AllowListedRoutes,
				...
				{
					Conditions: []contour_api_v1.MatchCondition{{
						Prefix: "/internal-redirect-denied",
					}},
					Services: []contour_api_v1.Service{{
						Name: "envoy-service",
						Port: 80,
				...
				},
				{
					...
					// only allows 301
					InternalRedirectPolicy: &contour_api_v1.HTTPInternalRedirectPolicy{
						RedirectResponseCodes: []uint32{301},
					},
				}},
		},
	}
